  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
      <div className="w-full max-w-2xl mx-auto flex flex-col gap-6">
          ...
          <CardHeader>
            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2"><Badge className="bg-blue-600 hover:bg-blue-700">STEP 1</Badge><span className="text-sm font-medium text-muted-foreground">Participant Verification</span></div>
                    <CardTitle className="text-2xl truncate">{member.name || "Participant"}</CardTitle>
                    <CardDescription className="mt-1 flex items-center gap-2 flex-wrap">{member.institution || "No Institution Set"}</CardDescription>
                </div>
                <div className="flex flex-row md:flex-col items-center md:items-end gap-2 w-full md:w-auto justify-between md:justify-start">
                    <div className="px-3 py-1 text-xs font-medium text-secondary-foreground border rounded-md bg-secondary/50 text-center">{getStatusText(status)}</div>
                ...
                <div key={member.account_id} className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-medium text-sm flex items-center gap-1.5 flex-wrap"><span className="truncate block max-w-[120px] sm:max-w-[200px]">{member.name || member.email}</span><span className="text-[10px] bg-blue-50 text-blue-600 font-bold px-1.5 py-0.5 rounded border border-blue-100 shrink-0">YOU</span></div>
                      {member.name && <p className="text-xs text-muted-foreground truncate">{member.email}</p>}
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0"><MemberStatusBadge status={status} notes={member.notes}/><PhotoMemberDialog member={member} /></div>
                </div>
          </CardContent>
          <Separator />
          <CardFooter className="pt-6 flex justify-between items-center gap-4"><span className="text-xs text-muted-foreground">Need to leave?</span><TeamLeaveButton action={leavePhoto} title="Photo Competition" /></CardFooter>
        </Card>
        ...
        {isSubmissionLocked ? (
            <LockedSection step="STEP 3" title="Submission & Originality" description="Submit originality proof and photo." subtext="Unlocks after Payment acceptance." borderColorClass="border-l-indigo-500" />
        ) : (
