...
import { PhotoMember } from "../types/Photo";

export async function registerUser(accountId: string, email: string) {
    const existing = await DB`SELECT 1 FROM photo_member WHERE account_id = ${accountId}`;
    if (existing.length > 0) return; 

    await DB`
        INSERT INTO photo_member (account_id, email, status)
        VALUES (${accountId}, ${email}, 0)
    `;
}

export async function unregisterUser(accountId: string) {
    await DB`DELETE FROM photo_member WHERE account_id = ${accountId}`;
}

export async function fetchPhotoPageData(accountId: string) {
    const result = await DB`SELECT * FROM photo_member WHERE account_id = ${accountId}`;
    if (result.length === 0) return null;
    return result[0] as PhotoMember;
}
