...
import { updateMemberDetails, getMemberDocuments, uploadMemberDocument } from "@/actions/server/photo";
...
export function PhotoMemberDialog({ member: initialMember }: { member: PhotoMember }) {
  const [open, setOpen] = useState(false);
  const hasHandledSuccess = useRef(false);
  const [member, setMember] = useState<PhotoMember>(initialMember);
  ...
  const inputClass = "bg-black/20 border-white/10 text-slate-200 placeholder:text-slate-500 focus-visible:ring-pink-500/50";
    ...
    >
      ...
      >
        <div className="p-6 pb-4 border-b border-white/10">
          <DialogHeader>
            <DialogTitle className="text-white">Edit Your Details</DialogTitle>
            <DialogDescription className="text-slate-400">Photo participant info.</DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <form id="smart-update-photo-form" onSubmit={handleSmartSubmit} className="h-full">
              <FieldGroup className="flex flex-col gap-8">
                
                <div className="space-y-4">
                  <h4 className="text-sm font-semibold text-slate-200 flex items-center gap-2">
                    <span className="bg-pink-500/20 text-pink-300 border border-pink-500/30 w-6 h-6 rounded-full flex items-center justify-center text-xs">1</span>
                    Personal Information
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <Field>
                        <FieldLabel className={labelClass}>Full Name</FieldLabel>
                        <Input className={inputClass} name="name" defaultValue={member.name ?? ""} placeholder="e.g. John Doe" />
                      </Field>
                      <Field>
                        <FieldLabel className={labelClass}>Institution</FieldLabel>
                        <Input className={inputClass} name="institution" defaultValue={member.institution ?? ""} placeholder="University" />
                      </Field>
                      <Field>
                        <FieldLabel className={labelClass}>Phone Number</FieldLabel>
                        <Input className={inputClass} name="phone_num" defaultValue={member.phone_num ?? ""} />
                      </Field>
                      <Field>
                        <FieldLabel className={labelClass}>Student ID Number</FieldLabel>
                        <Input className={inputClass} name="id_no" defaultValue={member.id_no ?? ""} />
                      </Field>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <h4 className="text-sm font-semibold text-slate-200 flex items-center gap-2">
                    <span className="bg-pink-500/20 text-pink-300 border border-pink-500/30 w-6 h-6 rounded-full flex items-center justify-center text-xs">2</span>
                    Documents
                  </h4>
                   ...
                   <div className="mt-4 pt-6 border-t border-white/10">
                    ...
                    <div className="flex flex-col gap-3">
                      {member.notes.map((note, idx) => (
                        <div key={idx} className="relative flex gap-3 rounded-lg border border-l-4 border-l-amber-400 bg-amber-900/10 border-y-amber-500/10 border-r-amber-500/10 p-4 text-sm shadow-sm">
                           <Info className="h-4 w-4 text-amber-400 shrink-0 mt-0.5" />
                           <span className="text-amber-100 leading-relaxed">{note}</span>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div className="p-6 pt-4 border-t border-white/10 bg-slate-950 mt-auto">
            <DialogFooter>
              ...
              <Button 
                type="submit" 
                form="smart-update-photo-form" 
                disabled={isSaving} 
                className="bg-pink-600 hover:bg-pink-500 text-white w-full sm:w-auto font-semibold border-none min-w-[120px]"
              >
