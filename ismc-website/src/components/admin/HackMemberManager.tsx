...
import { updateHackStatus, getSignedDocUrl } from "@/actions/server/admin_hackathon";
...
export function HackMemberManager({ member }: { member: MemberHack }) {
  ...
  const handleUpdateDoc = async (field: string, value: number) => {
    ...
    const res = await updateHackStatus(member.account_id, 'member', field, value);
  ...
  const handleMemberStatus = async (newStatus: number) => {
      ...
      const res = await updateHackStatus(member.account_id, 'member', 'status', newStatus);
  ...
  const handleAddNote = async (formData: FormData) => {
      ...
      if(!note) return;
      await updateHackStatus(member.account_id, 'member', 'notes', note, 'update');
      toast.success("Note added");
  };

  const handleDeleteNote = async (note: string) => {
      await updateHackStatus(member.account_id, 'member', 'notes', note, 'remove_note');
    ...
    `}>
      ...
      <div className="flex justify-between items-start mb-4 pb-3 border-b border-zinc-800">
        <div className="flex items-center gap-3">
            <div className={`h-8 w-8 rounded-full flex items-center justify-center ${member.role === 'MANAGER' ? 'bg-blue-500/10 text-blue-500 border border-blue-500/20' : 'bg-zinc-800 text-zinc-500'}`}>
