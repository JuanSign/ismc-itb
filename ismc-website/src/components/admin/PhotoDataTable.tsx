...
export function PhotoDataTable({ data }: { data: PhotoMember[] }) {
  ...
  const handleUpdateStatus = async (id: string, value: string) => {
      ...
      const res = await updatePhotoStatus(id, 'status', Number(value));
  ...
  const handleAddNote = async (id: string, formData: FormData) => {
      ...
      if(!note) return;
      await updatePhotoStatus(id, 'notes', note, 'update');
      toast.success("Note added");
  };

  const handleDeleteNote = async (id: string, note: string) => {
      await updatePhotoStatus(id, 'notes', note, 'remove_note');
              ...
              filteredData.map((m) => (
                <Fragment key={m.account_id}>
                  ...
                  >
                    ...
                    <TableCell>
                        {m.sub_verified === 2 ? <Badge variant="outline" className="bg-pink-500/10 text-pink-400 border-pink-500/30">Accepted</Badge>
                  ...
                  {expandedRows.has(m.account_id) && (
                    <TableRow className="bg-black border-b border-zinc-800 hover:bg-black">
                      <TableCell colSpan={6} className="p-0">
                        <div className="p-6 border-l-2 border-pink-600 bg-zinc-950/30 shadow-inner">
                                    ...
                                    <div className="border-t border-zinc-800 pt-6">
                                        ...
                                        <div className="bg-zinc-900/20 p-4 rounded border border-zinc-800/50">
                                            ...
                                            <div className="flex items-center gap-2 mb-2 text-pink-400">
                                                ...
                                                <h5 className="text-[10px] font-bold uppercase">Caption / Theme</h5>
                                            </div>
                                            <div className="mb-4 p-3 bg-black rounded border border-zinc-800 text-xs text-zinc-300 max-h-32 overflow-y-auto custom-scrollbar whitespace-pre-wrap leading-relaxed">
                                                {m.sdd ? m.sdd : <span className="text-zinc-600 italic">No caption submitted yet.</span>}
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    <h6 className="text-[10px] text-zinc-500 uppercase font-bold mb-2">Photography</h6>
                                                    <DocRow label="Photo File" link={m.sd_link} verified={m.sub_verified} field="sub_verified" id={m.account_id} />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
