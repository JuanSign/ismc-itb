...
import { updatePaperStatus, getSignedDocUrl } from "@/actions/server/admin_paper";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { Check, X, ExternalLink, Loader2, User, Trash2, Send } from "lucide-react";
import { toast } from "sonner";
import { MemberPaper } from "@/actions/types/Paper";

export function PaperMemberManager({ member }: { member: MemberPaper }) {
  ...
  const handleUpdateDoc = async (field: string, value: number) => {
    ...
    const res = await updatePaperStatus(member.account_id, 'member', field, value);
  ...
  const handleMemberStatus = async (newStatus: number) => {
      ...
      const res = await updatePaperStatus(member.account_id, 'member', 'status', newStatus);
  ...
  const handleAddNote = async (formData: FormData) => {
      ...
      if(!note) return;
      await updatePaperStatus(member.account_id, 'member', 'notes', note, 'update');
      toast.success("Note added");
  };

  const handleDeleteNote = async (note: string) => {
      await updatePaperStatus(member.account_id, 'member', 'notes', note, 'remove_note');
    ...
    `}>
      ...
      <div className="flex justify-between items-start mb-4 pb-3 border-b border-zinc-800">
        <div className="flex items-center gap-3">
            <div className={`h-8 w-8 rounded-full flex items-center justify-center ${member.role === 'MANAGER' ? 'bg-purple-500/10 text-purple-500 border border-purple-500/20' : 'bg-zinc-800 text-zinc-500'}`}>
