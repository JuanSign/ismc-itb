...
export function PaperDataTable({ data }: { data: PaperTeam[] }) {
  ...
  const handleUpdateTeamStatus = async (teamId: string, value: string) => {
      ...
      const res = await updatePaperStatus(teamId, 'team', 'status', Number(value));
  ...
  const handleAddNote = async (teamId: string, formData: FormData) => {
      ...
      if(!note) return;
      await updatePaperStatus(teamId, 'team', 'notes', note, 'update');
      toast.success("Note added");
  };

  const handleDeleteNote = async (teamId: string, note: string) => {
      await updatePaperStatus(teamId, 'team', 'notes', note, 'remove_note');
      toast.success("Note deleted");
  };

  const handleVerifyTeamDoc = async (teamId: string, field: string, value: number) => {
      await updatePaperStatus(teamId, 'team', field, value);
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                                ...
                                <div className="lg:col-span-4 space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <TeamDocRow label="Proof of Originality" link={team.od_link} verified={team.od_verified} field="od_verified" teamId={team.team_id} />
                                ...
                                <div className="lg:col-span-8">
                                    ...
                                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                                        {team.members.map((m) => (
                                            <PaperMemberManager key={m.account_id} member={m} />
                                        ))}
                                    </div>
                                </div>
